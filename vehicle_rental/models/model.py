...
class FleetVehicle(models.Model):
    ...
    def set_insurance_cron(self):
        for rec in self:
            ...
            if rec.insurance_expiration_date and rec.company_id.fleet_vehicle_expiration_notification_user:
...
vehicle_id.write({{'activity_ids': [(0, 0, {{
    ...
    'user_id': vehicle_id.company_id.fleet_vehicle_expiration_notification_user.id
}})]}})
                        """,
                    'nextcall': datetime(
    ...
    def set_registration_cron(self):
        for rec in self:
            ...
            if rec.registration_expiration_date and rec.company_id.fleet_vehicle_expiration_notification_user:
...
vehicle_id.write({{'activity_ids': [(0, 0, {{
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['analytic_tag_ids'] = [(0, 0, {
                'name': vals['license_plate'],
                'plan_id': self.env.ref('vehicle_rental.analytic_plan_rent').id
            })]
    ...
    def write(self, vals):
        ...
        if 'insurance_expiration_date' in vals:
            self.set_insurance_cron()
        if 'registration_expiration_date' in vals:
